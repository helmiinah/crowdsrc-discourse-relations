...
class AnalyticsObserver(BaseObserver):
    def __init__(self, client, pool, **options) -> None:
        ...
        self.pool = pool
        self.limit = (
            options["max_performers"]
            if options and "max_performers" in options
            else None
        )
            ...
            if operation.status == Operation.Status.SUCCESS:
                ...
                for response in operation.details["value"]:
                    ...
                    if self.limit and not self.limit_reached:
                        ...
                        if (
                            response["request"]["name"] == "unique_workers_count"
                            and response["result"] >= self.limit
                        ):

                            msg.warn(
                                f"Maximum number of performers ({self.limit}) reached for "
                            ...
                            try:
                                ...
                                for training in await self.client.get_trainings(
                                    project_id=self.pool.project_id
                                ):
                                    await self.client.close_training_async(
                                        training_id=training.id
                                    )
                                    msg.good(
                                        f"Successfully closed training pool {training.id}"
                                    )
                                ...
                                msg.warn(
                                    f"Attempted to close a training for pool {self.pool.id} that does not exist"
                                )
                    ...
                    if (
                        ...
                        if response["result"] != self.prev_result:
                            ...
                            msg.info(
                                f'{response["result"]} workers submitted to pool {self.pool.id}'
                            )
    ...
    async def create_operation(self):
        ...
        stat_requests = [
            ...
            ActiveWorkersByFilterCountPoolAnalytics(
                subject_id=self.pool.id, interval_hours=1
            ),

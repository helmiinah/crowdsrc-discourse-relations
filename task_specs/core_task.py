...
from toloka.client.pool.speed_quality_balance_config import (
...
class CrowdsourcingTask:
    ...
    def __init__(self, configuration, client, task_spec):
        ...
        self.action_conf = (
            self.conf["actions"] if "actions" in self.conf.keys() else None
        )
        self.pool_conf = self.conf["pool"]
        self.train_conf = (
            self.conf["training"] if "training" in self.conf.keys() else None
        )
        self.qual_conf = (
            self.conf["quality_control"]
            if "quality_control" in self.conf.keys()
            else None
        )
        ...
        try:
            self.blocklist = (
                list(pd.read_csv(self.pool_conf["blocklist"], sep="\t")["user_id"])
                if "blocklist" in self.pool_conf.keys()
                else []
            )
        except KeyError:
    ...
    def __call__(self, in_obj, **options):
        ...
        if type(in_obj) == list and all(
            isinstance(item, AssignmentEvent) for item in in_obj
        ):
                ...
                if event.event_type.value in ["ACCEPTED", "SUBMITTED"]:
                    ...
                    new_tasks = [
                        Task(
                            ...
                            overlap=self.pool_conf["defaults"][
                    ...
                    msg.good(
                        f"Received {len(new_tasks)} {event.event_type.value.lower()} tasks "
                        f"from pool {event.assignment.pool_id}"
                    )
                    msg.good(
                        f"Creating {len(new_tasks)} new tasks in pool {self.pool.id}"
                    )
    ...
    def load_project(self, client, task_spec):
        ...
        if "id" in self.project_conf.keys():
            ...
            # Catch the error
                ...
                # Raise error
                raise_error(
                    f'Failed to load project with ID {self.project_conf["id"]} from '
        ...
        # Otherwise create new project
            ...
            try:
                ...
                with open(
                    self.project_conf["instructions"], mode="r", encoding="utf-8"
                ) as inst:
            ...
            # Print status message
            msg.good(
                f"Successfully created a new project with ID {self.project.id} on Toloka"
            )
        ...
        if self.train_conf is not None:
            ...
            if "id" in self.train_conf.keys():
                ...
                try:
                    ...
                    self.training = client.get_training(
                        training_id=self.train_conf["id"]
                    )

                    # Print status message
                    msg.good(
                        f"Successfully loaded training pool with ID {self.training.id} "
                        f"from Toloka"
                    )
                ...
                # Catch the error
                    ...
                    # Raise error
                    raise_error(
                        f'Failed to load training pool with ID {self.train_conf["id"]} '
                        f"from Toloka"
                    )
            ...
            # Otherwise create a new training pool
                ...
                self.train_data = load_data(
                    self.conf["training"]["data"]["file"],
                    self.conf["training"]["data"]["input"],
                )

                # Get the input and output variable names
                input_values = {
                    n: n for n in list(self.conf["training"]["data"]["input"].keys())
                }
                output_values = {
                    n: n for n in list(self.conf["training"]["data"]["output"].keys())
                }

                # Create Task objects for training
                self.train_tasks = [
                    toloka.Task(
                        pool_id=self.training.id,
                        input_values={k: row[v] for k, v in input_values.items()},
                        known_solutions=[
                            toloka.task.BaseTask.KnownSolution(
                                output_values={
                                    k: str(row[v]) for k, v in output_values.items()
                                }
                            )
                        ],
    ...
    def load_pool(self, client):
        ...
        if "id" in self.pool_conf.keys():
            ...
            try:
                ...
                # Print status message
                msg.good(
                    f"Successfully loaded main pool with ID {self.pool.id} from Toloka"
                )
            ...
            # Catch the error
                ...
                # Raise error
                raise_error(
                    f'Failed to load pool with ID {self.pool_conf["id"]} from Toloka'
                )
        ...
        # Otherwise proceed to create a new main pool
            ...
            if self.train_conf is not None:
                ...
                self.pool.set_training_requirement(
                    training_pool_id=self.training.id, **self.pool_conf["training"]
                )
            ...
            if "estimated_time_per_suite" not in self.pool_conf.keys():
                msg.warn(
                    f"Parameter estimated_time_per_suite is not configured in pool settings for {self.name}. "
                ...
                check_reward(
                    self.pool_conf["estimated_time_per_suite"],
            ...
            if (
                "speed_quality_balance" in self.pool_conf.keys()
                and self.pool_conf["speed_quality_balance"] is not None
            ):

                if (
                    "top_percentage_by_quality"
                    in self.pool_conf["speed_quality_balance"]
                ):

                    # Get percentage for top workers from configuration
                    user_percentage = self.pool_conf["speed_quality_balance"][
                        "top_percentage_by_quality"
                    ]
                    ...
                    msg.info(
                        f"Speed/quality setting: only top {user_percentage}% of users will have access to pool {self.name}"
                    )
                    ...
                    user_count = self.pool_conf["speed_quality_balance"][
                        "best_concurrent_users_by_quality"
                    ]
                    ...
                    msg.info(
                        f"Speed/quality setting: only top {user_count} users will have access to pool {self.name}"
                    )
            ...
            if (
                "filter" in self.pool_conf.keys()
                and self.pool_conf["filter"] is not None
            ):
                ...
                if "languages" in self.pool_conf["filter"].keys():
                    ...
                    if len(self.pool_conf["filter"]["languages"]) == 1:
                        ...
                        self.pool.filter = set_filter(
                            filters=self.pool.filter, new_filters=language
                        )
                    ...
                    if len(self.pool_conf["filter"]["languages"]) > 1:
                        ...
                        # Create filters
                        languages = [
                            (toloka.filter.Languages.in_(lang.upper()))
                        ...
                        # Check for existing filters and set
                        self.pool.filter = set_filter(
                            filters=self.pool.filter, new_filters=languages
                        )
                ...
                if "client_type" in self.pool_conf["filter"].keys():
                    ...
                    if len(self.pool_conf["filter"]["client_type"]) == 1:
                        ...
                        # Check for existing filters and set
                        self.pool.filter = set_filter(
                            filters=self.pool.filter, new_filters=client
                        )
                    ...
                    if len(self.pool_conf["filter"]["client_type"]) > 1:
                        ...
                        # Create filters
                        clients = [
                            (toloka.filter.ClientType == client.upper())
                        ...
                        # Check for existing filters and set
                        self.pool.filter = set_filter(
                            filters=self.pool.filter, new_filters=clients
                        )
                ...
                if "education" in self.pool_conf["filter"].keys():
                    ...
                    if len(self.pool_conf["filter"]["education"]) == 1:
                        ...
                        # Create filter
                        education = (
                            toloka.filter.Education
                            == self.pool_conf["filter"]["education"][0].upper()
                        )

                        # Check for existing filters and set
                        self.pool.filter = set_filter(
                            filters=self.pool.filter, new_filters=education
                        )
                    ...
                    if len(self.pool_conf["filter"]["education"]) > 1:
                        ...
                        # Create filters
                        levels = [
                            (toloka.filter.Education == edu.upper())
                            for edu in self.pool_conf["filter"]["education"]
                        ]
                        ...
                        # Check for existing filters and set
                        self.pool.filter = set_filter(
                            filters=self.pool.filter, new_filters=levels
                        )
                ...
                if "skill" in self.pool_conf["filter"].keys():
                    ...
                    if len(self.pool_conf["filter"]["skill"]) == 1:
                        ...
                        skill_id = str(
                            list(self.pool_conf["filter"]["skill"][0].keys())[0]
                        )
                        skill_value = int(
                            list(self.pool_conf["filter"]["skill"][0].values())[0]
                        )
                        ...
                        # Check for existing filters and set
                        self.pool.filter = set_filter(
                            filters=self.pool.filter, new_filters=skill
                        )
                    ...
                    if len(self.pool_conf["filter"]["skill"]) > 1:
                        ...
                        skills = [
                            (
                                toloka.filter.Skill(str(list(skill_dict.keys())[0]))
                ...
                if "gender" in self.pool_conf["filter"].keys():
                    ...
                    self.pool.filter = set_filter(
                        filters=self.pool.filter, new_filters=gender
                    )
                ...
                if "adult_allowed" in self.pool_conf["filter"].keys():
                    ...
                    self.pool.filter = set_filter(
                        filters=self.pool.filter, new_filters=adult_allowed
                    )
                ...
                if "country" in self.pool_conf["filter"].keys():
                    ...
                    if len(self.pool_conf["filter"]["country"]) == 1:
                        ...
                        # Create filter
                        country = (
                            toloka.filter.Country
                            == self.pool_conf["filter"]["country"][0].upper()
                        )

                        # Check for existing filters and set
                        self.pool.filter = set_filter(
                            filters=self.pool.filter, new_filters=country
                        )
                    ...
                    if len(self.pool_conf["filter"]["country"]) > 1:
                        ...
                        # Create filters
                        levels = [
                            (toloka.filter.Country == c.upper())
                            for c in self.pool_conf["filter"]["country"]
                        ]
                ...
                if "city" in self.pool_conf["filter"].keys():
                    ...
                    if len(self.pool_conf["filter"]["city"]) == 1:
                        ...
                        # Create filter
                        city = toloka.filter.City == int(
                            self.pool_conf["filter"]["city"][0]
                        )

                        # Check for existing filters and set
                        self.pool.filter = set_filter(
                            filters=self.pool.filter, new_filters=city
                        )
                    ...
                    if len(self.pool_conf["filter"]["city"]) > 1:
                        ...
                        # Create filters
                        levels = [
                            (toloka.filter.City == int(c))
                            for c in self.pool_conf["filter"]["city"]
                        ]
                ...
                if "date_of_birth" in self.pool_conf["filter"].keys():
                    ...
                    if len(self.pool_conf["filter"]["date_of_birth"]) == 1:
                        ...
                        if "before" in self.pool_conf["filter"]["date_of_birth"].keys():
                            ...
                            self.pool.filter = set_filter(
                                filters=self.pool.filter, new_filters=b_filter
                            )
                        ...
                        if "after" in self.pool_conf["filter"]["date_of_birth"].keys():
                            ...
                            self.pool.filter = set_filter(
                                filters=self.pool.filter, new_filters=a_filter
                            )
                    ...
                    if len(self.pool_conf["filter"]["date_of_birth"]) > 1:
                        ...
                        ba_filters = [
                            (toloka.filter.DateOfBirth <= before),
                            (toloka.filter.DateOfBirth >= after),
                        ]
                ...
                if "user_agent_type" in self.pool_conf["filter"].keys():
                    ...
                    if len(self.pool_conf["filter"]["user_agent_type"]) == 1:
                        ...
                        # Create filter
                        agent = (
                            toloka.filter.UserAgentType
                            == self.pool_conf["filter"]["user_agent_type"][0].upper()
                        )

                        # Check for existing filters and set
                        self.pool.filter = set_filter(
                            filters=self.pool.filter, new_filters=agent
                        )
                    ...
                    if len(self.pool_conf["filter"]["user_agent_type"]) > 1:
                        ...
                        # Create filters
                        levels = [
                            (toloka.filter.UserAgentType == agent.upper())
                            for agent in self.pool_conf["filter"]["user_agent_type"]
                        ]
            ...
            if "skill" in self.pool_conf.keys():
                ...
                if "id" in self.pool_conf["skill"].keys():
                    ...
                    try:
                        ...
                        # Retrieve the skill
                        self.skill = client.get_skill(
                            skill_id=self.pool_conf["skill"]["id"]
                        )

                        # Print status message
                        msg.good(
                            f"Successfully loaded skill with ID {self.skill.id} from Toloka"
                        )
                    ...
                    # Catch the error
                        ...
                        # Raise error
                        raise_error(
                            f'Failed to load skill with ID {self.pool_conf["skill"]["id"]} '
                            f"from Toloka"
                        )
                ...
                # Otherwise create a new skill
                    ...
                    self.skill = client.create_skill(
                        name=self.pool_conf["skill"]["name"],
                        public_requester_description={
                            self.pool_conf["skill"]["language"]: self.pool_conf[
                                "skill"
                            ]["description"]
                        },
                    )
            ...
            if self.qual_conf is not None:
                ...
                if "fast_responses" in self.qual_conf:
                    ...
                    # Print status message
                    msg.good(
                        f"Added quality control rule: ban for {duration} {units.lower()} if "
                ...
                if "redo_banned" in self.qual_conf:
                    ...
                    if self.qual_conf["redo_banned"]:
                        ...
                        self.pool.quality_control.add_action(
                            ...
                            conditions=[
                                toloka.conditions.PoolAccessRevokedReason
                                == toloka.conditions.PoolAccessRevokedReason.RESTRICTION
                            ],
                            action=toloka.actions.ChangeOverlap(
                                delta=1, open_pool=True
                            ),
                        )

                        # Print status message
                        msg.good(
                            f"Assignments from banned users will be re-added to the pool."
                        )
                ...
                if "golden_set" in self.qual_conf:
                    ...
                    if "ban_rules" in self.qual_conf["golden_set"]:
                        ...
                        # Unpack rules into variables
                        incorrect_threshold = self.qual_conf["golden_set"]["ban_rules"][
                            "incorrect_threshold"
                        ]
                        ban_duration = self.qual_conf["golden_set"]["ban_rules"][
                            "ban_duration"
                        ]
                        ban_units = self.qual_conf["golden_set"]["ban_rules"][
                            "ban_units"
                        ]
                        ...
                        self.pool.quality_control.add_action(
                            ...
                            conditions=[
                                toloka.conditions.GoldenSetIncorrectAnswersRate
                                > incorrect_threshold
                            ],
                            action=toloka.actions.RestrictionV2(
                                scope=toloka.user_restriction.UserRestriction.PROJECT,
                        ...
                        # Print status message
                        msg.good(
                            f"Added quality control rule: ban for {ban_duration} {ban_units.lower()} if "
                    ...
                    if "reject_rules" in self.qual_conf["golden_set"]:
                        ...
                        incorrect_threshold = self.qual_conf["golden_set"][
                            "reject_rules"
                        ]["incorrect_threshold"]
                        ...
                        # Print status message
                        msg.good(
                            f"Added quality control rule: Reject all user's assignments if "
                            f"user fails over {incorrect_threshold}% of control tasks."
                        )
                    ...
                    if "approve_rules" in self.qual_conf["golden_set"]:
                        ...
                        correct_threshold = self.qual_conf["golden_set"][
                            "approve_rules"
                        ]["correct_threshold"]
                        ...
                        self.pool.quality_control.add_action(
                            ...
                            conditions=[
                                toloka.conditions.GoldenSetCorrectAnswersRate
                                > correct_threshold
                            ],
                        ...
                        # Print status message
                        msg.good(
                            f"Added quality control rule: Approve all user's assignments if "
                    ...
                    if "skill_rules" in self.qual_conf["golden_set"]:
                        ...
                        # Unpack rules into variables
                        correct_threshold = self.qual_conf["golden_set"]["skill_rules"][
                            "correct_threshold"
                        ]
                        skill_id = self.qual_conf["golden_set"]["skill_rules"][
                            "skill_id"
                        ]
                        skill_value = self.qual_conf["golden_set"]["skill_rules"][
                            "skill_value"
                        ]
                        ...
                        self.pool.quality_control.add_action(
                            ...
                            action=toloka.actions.SetSkill(
                                skill_id=skill_id, skill_value=skill_value
                            ),
                        )

                        # Print status message
                        msg.good(
                            f"Added quality control rule: Grant user a skill value {skill_value} for skill "
            ...
            if "exam" in self.pool_conf.keys():
                ...
                if not self.skill:
                    ...
                    msg.fail(
                        f'The pool configuration contains the key "exam", but no skill has '
                        f"been defined in the pool configuration.",
                        exits=0,
                    )
                ...
                if "max_performers" not in self.pool_conf["exam"]:
                    ...
                    # Print status message and exit
                    msg.fail(
                        f'The pool configuration contains the key "exam", but the number of '
                ...
                # answers only
                if (
                    not self.pool_conf["mixer"]["real_tasks_count"] == 0
                    and self.pool_conf["mixer"]["training_tasks_count"] == 0
                ):

                    # Print status message and exit
                    msg.fail(
                        f"The configuration file defines an exam pool, but the count for both "
                        f"real and training tasks is greater than 0. Exam pools must contain "
                        f"tasks with known (golden) answers only.",
                        exits=0,
                    )
                ...
                if "file" not in self.data_conf:
                    ...
                    # Print status message and exit
                    msg.fail(
                        f'The data configuration does not contain an entry for "file", which '
                        f"should define a path to a TSV with exam tasks.",
                        exits=0,
                    )
                ...
                self.pool.quality_control.add_action(
                    ...
                    conditions=[
                        toloka.conditions.TotalAnswersCount
                        >= self.pool_conf["exam"]["min_answers"]
                    ],
                    action=toloka.actions.SetSkillFromOutputField(
                        skill_id=self.skill.id, from_field="correct_answers_rate"
                    ),
                )

                # Print status message
                msg.good(
                    f"Successfully configured exam pool using skill {self.skill.id}"
                )
            ...
            # Print status message
            msg.good(
                f"Successfully created a new pool with ID {self.pool.id} on Toloka"
            )

...
def find_gold_standard(row):
    d = [
        value for dic in structures for (key, value) in dic.items() if key == f"{row['id']}.png"
    ]
...
df = pd.read_csv("analysis/mace/results/prediction_balanced", index_col=False, header=None)
